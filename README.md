...
> Обновление приложения, команда update
  ...
  # Что делает CLI-команда update при обновлении приложения
    ...
    • Определяет название группы конфигов для запуска приложения
    • Определяет, запускать ли проект после обновления
    • Удаляет все образы проекта
    • Получает URL репозитория проекта, проверить его
    • Проверить существование .gitignore в корне проекта
    • Скачивает новые файлы проекта, заменить ими старые
    • Сообщает об успешном обновлении
    • Запускает проект, если не была передана опция -n    
